...
log_dirs = {'myPC':'E:/logs/dataloaderv2.0',
            'myLaptop': '/media/furqan/Data/traininig_logs'} # add more for servers
...
def pl_train():
    ...
    # trainer
    trainer = pl.Trainer(gpus=-1, auto_scale_batch_size=False,enable_checkpointing=True,precision=16,logger=wandb,
                         callbacks=[checkpoint, bar], max_epochs=10 )

    trainer.fit(model, dataset)



def pl_inference(PATH):
    ...
    # logger
    wandb = WandbLogger(project='NuscAug')
    checkpoint = pl.callbacks.ModelCheckpoint(dirpath=os.path.join(log_dirs['myPC'], 'checkpoints'), verbose=True, )
    bar = pl.callbacks.TQDMProgressBar()
